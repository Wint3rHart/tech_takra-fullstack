...
const UnsplashPics = ({ pics }) => {
    ...
    const handleSlideTransition = useCallback(async (direction = 'next') => {
        if (isTransitioning || !pics?.length) return;
        ...
        const nextIndex = direction === 'next' 
            ...
            : (currentIndex - 1 + pics.length) % pics.length;
    ...
    }, [currentIndex, pics, animate, isTransitioning]);
    ...
    }, [currentIndex, handleSlideTransition, pics]);
    ...
    return (
        <div ref={scope} className='relative h-screen w-full overflow-hidden bg-black'>
            {/* Navigation Controls */}
