      ...
      <div className="relative z-10 p-6 sm:p-8 space-y-6">
        ...
        <div className="flex flex-wrap items-start gap-6 justify-between border-b border-amber-600/20 pb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-amber-400/70 font-poppins font-semibold">Registrant</p>
              <h3 className="text-2xl font-inter font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] to-amber-200 drop-shadow-[1px_1px_2px_rgba(212,175,55,0.3)]">
                {data?.name}
              </h3>
              <p className="text-sm text-gray-400 font-poppins font-medium">{data?.department}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs uppercase tracking-[0.3em] text-amber-400/70 font-poppins font-semibold">Submitted</p>
            <p className="text-base text-amber-200 font-poppins font-semibold">{formattedDate}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-800/60 border border-amber-600/20 rounded-2xl p-4">
            <p className="text-xs uppercase tracking-[0.3em] text-amber-400/70 font-poppins font-semibold mb-2">Roll Number</p>
            <p className="text-lg text-gray-200 font-inter font-normal">{data?.rollNo}</p>
          </div>
          <div className="bg-gray-800/60 border border-amber-600/20 rounded-2xl p-4">
            <p className="text-xs uppercase tracking-[0.3em] text-amber-400/70 font-poppins font-semibold mb-2">Department</p>
            <p className="text-lg text-gray-200 font-inter font-normal">{data?.department}</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-800/40 rounded-2xl border border-amber-600/20 p-4">
            <p className="text-xs uppercase tracking-[0.3em] text-amber-400/70 font-poppins font-semibold mb-1">Phone</p>
            <p className="text-lg text-amber-200 font-inter font-normal">{data?.phone}</p>
          </div>
          <div className="bg-gray-800/40 rounded-2xl border border-amber-600/20 p-4">
            <p className="text-xs uppercase tracking-[0.3em] text-amber-400/70 font-poppins font-semibold mb-1">Email</p>
            <p className="text-base text-gray-100 font-inter font-normal break-all">{data?.email}</p>
          </div>
        </div>
