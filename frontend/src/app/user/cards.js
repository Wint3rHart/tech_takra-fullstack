...
const Cards = React.memo(({booking,i}) => {
    const {abort_ref,post,msg}=usePost("delete_booking","DELETE");
const {mutate,error,isSuccess}=post;
    ...
    const del_fnx=(booking_id)=>{
console.log(booking_id);
mutate({booking_id:booking_id});
                            ...
                            <div 
                                ...
                                {booking.isPackage === 1 && (
                                    <div className="absolute top-4 right-4 z-20">
                                        <span className="px-3 py-1 bg-gradient-to-r from-amber-500 to-[#d4af37] 
                                                       text-gray-900 text-xs font-bold rounded-full
                                                       shadow-lg shadow-amber-400/50 border border-amber-600/30">
                                            PACKAGE
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="mb-4 relative z-10">
                                    ...
                                    <div className="flex items-center gap-2 text-gray-300 ml-8">
                                        ...
                                        <span className="font-semibold text-sm">{booking.hotel_name}</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-5 relative z-10">
                                    ...
                                    <div className="bg-gray-800/50 rounded-xl p-3 border border-amber-600/20
                                        ...
                                        <p className="text-lg font-bold text-amber-300">{booking.duration} Days</p>
                                    </div>
                                    ...
                                    <div className="bg-gray-800/50 rounded-xl p-3 border border-amber-600/20
                                        ...
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-xs text-gray-400 font-semibold">Total Price</span>
                                        </div>
                                ...
                                {booking.requests && (
                                    <div className="mb-4 p-3 bg-gray-800/30 rounded-xl border border-amber-600/10 relative z-10">
                                        ...
                                        <p className="text-sm text-gray-300">{booking.requests}</p>
                                    </div>
                                )}

                                {/* View Details Button */}
                                <button onClick={()=>{del_fnx(booking.id)}} className="w-full mt-4 px-6 py-3 rounded-xl font-bold text-gray-900
